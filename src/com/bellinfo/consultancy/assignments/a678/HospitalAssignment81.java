...
public class HospitalAssignment81 {
	public static void main(String[] args) {
		...
		Scanner scan = new Scanner(System.in);
	 	int zipper = scan.nextInt();
		int temp = 5000;
		int a = 4900;
		Hospital hospit = null;
		Iterator<Hospital> it = hs1.iterator();
		while (it.hasNext()) {
			Hospital hospi = (Hospital) it.next();
			if (hospi.getDoc().getSpecialization().equals("gynocology")) {

				if (hospi.getHospitalAddress().getZipCode() > zipper) {
					a = hospi.getHospitalAddress().getZipCode() - zipper;
				}
				if (hospi.getHospitalAddress().getZipCode() < zipper) {
					a = zipper - hospi.getHospitalAddress().getZipCode();
				}
				if (hospi.getHospitalAddress().getZipCode() == zipper) {
					a = 0;
				}
		...
		System.out.println(hospit.toString());
